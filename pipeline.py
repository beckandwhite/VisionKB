#!/usr/bin/env python3
"""Unified screenshot classifier + knowledgebase pipeline.

Single entry point. Per image it interleaves the former two-stage flow so the
KB stays live:

  reconcile folder -> _tracker.json
  for each unprocessed image:
      vision + embedding (Ollama)  -> append _annotations.jsonl
      ingest into SQLite wiki.db (FTS5 + embeddings)
      generate 320px thumbnail
      rebuild derived tables + exports (cheap, keeps the UI live)

Nightly cron: ./run.sh (time-boxed, resumable, flock-guarded).
Manual:        ./run.sh --count N --no-thumbs   (see README).
"""
