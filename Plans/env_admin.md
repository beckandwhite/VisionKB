...
- **Named envs** = `.workspace/<ENV>/` subfolders. The existing `DEV`, `QA`,
  `PRD-iCloud-Screenshots`, `PRD-OneDrive-Pictures` are kept as-is.
   - **Single source of truth**: a new `config.template.json` replaces
...
## 1. Template + `config_loader.py` refactor
   ...
   - **Add `config.template.json` at the repo root**: the full config schema **plus a
  complete `TAG_LIST`**. It is the only place defaults are written.
- **`config_loader.py`**:
       - `load_defaults()` reads the root `config.template.json`; delete the hard-coded
